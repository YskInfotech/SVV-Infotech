...
const Rejectcandidates = () => {
  // example varOcg placeholder (per your requirement)
  const varOcg = "reject_candidates_doc";
  ...
  // selectedRows now stores numeric ids
  ...
  // delete selected by id (with confirm)
  ...
  const handleDownloadSelected = async () => {
    ...
    try {
      ...
      const fname = `rejected_candidates_${new Date().toISOString().slice(0,19).replace(/[:T]/g, "-")}.docx`;
      saveAs(blob, fname);
    } catch (err) {
